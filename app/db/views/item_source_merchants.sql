...
WHERE (ml.min_expansion <= pok_get_eqemu_expansion())
  AND (ml.max_expansion = -1 OR ml.max_expansion >= pok_get_eqemu_expansion())
  AND EXISTS (SELECT 1 FROM pok_spawn_variants pv WHERE pv.npc_id = nt.id)
) as dt;
