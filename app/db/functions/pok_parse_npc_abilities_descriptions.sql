CREATE FUNCTION `pok_parse_npc_abilities_descriptions`(sa TEXT, p_category VARCHAR(16))
RETURNS TEXT
DETERMINISTIC
READS SQL DATA
BEGIN
  ...
  parse_loop: LOOP
    ...
    IF id_text REGEXP '^[0-9]+$' THEN
      ...
      IF ability_id > 0 AND FIND_IN_SET(ability_id, seen_ids) = 0 THEN
        ...
        IF matched > 0 AND ability_details IS NOT NULL THEN
          SET seen_ids = IF(seen_ids = '', id_text, CONCAT(seen_ids, ',', id_text));
          SET result   = IFNULL(result, '');
          SET result   = IF(result = '', ability_details, CONCAT(result, '|', ability_details));
        END IF;
      END IF;
    END IF;
