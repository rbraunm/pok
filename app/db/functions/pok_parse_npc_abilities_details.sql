CREATE FUNCTION `pok_parse_npc_abilities_details`(sa TEXT, p_category VARCHAR(16))
RETURNS TEXT
DETERMINISTIC
READS SQL DATA
BEGIN
  ...
  DECLARE ability_details TEXT;
  DECLARE parsed_piece TEXT;
  DECLARE matched INT;
  ...
  parse_loop: LOOP
    ...
    IF id_text REGEXP '^[0-9]+$' THEN
      ...
      IF ability_id > 0 AND FIND_IN_SET(ability_id, seen_ids) = 0 THEN
        ...
        SET ability_details = NULL;

        SELECT name, details
          INTO ability_name, ability_details
        FROM pok_eqemu_npc_abilities
        WHERE id = ability_id
        ...
        IF matched > 0 AND ability_name IS NOT NULL THEN
          -- Basic parsed piece: name + generic description + raw token for this NPC
          SET parsed_piece = CONCAT(ability_name, ': ', ability_details);

          IF token IS NOT NULL AND token <> '' THEN
            SET parsed_piece = CONCAT(parsed_piece, ' [', token, ']');
          END IF;

          SET seen_ids = IF(seen_ids = '', id_text, CONCAT(seen_ids, ',', id_text));
          SET result   = IFNULL(result, '');
          SET result   = IF(result = '', parsed_piece, CONCAT(result, '|', parsed_piece));
        END IF;
      END IF;
    END IF;
