...
INDEX_DEFS = [
  ...
  ("spawnentry", "group_minmax_chance", ["spawngroupID","min_expansion","max_expansion","chance"]),
  ...
  ("spawn2", "group_minmax", ["spawngroupID", "min_expansion", "max_expansion"]),
...
def _parseColSpec(colSpec):
  ...
  m = _len_re.match(colSpec.strip());  assert m, f"Bad column spec: {colSpec}"
  col = m.group(1); sub = int(m.group(2)) if m.group(2) else None
  return col, sub

def _desiredMap():
  desired = {}
  for table, base, cols in INDEX_DEFS:
...
def _getColMeta(cur):
  ...
  for r in (cur.fetchall() or []):
    if isinstance(r, dict):
      ...
      char_len = r["CHARACTER_MAXIMUM_LENGTH"]; octet_len = r["CHARACTER_OCTET_LENGTH"]
      dtype = (r["DATA_TYPE"] or "").lower()
    else:
      tbl, col, char_len, octet_len, dtype = r; dtype = (dtype or "").lower()
    try: char_len = int(char_len) if char_len is not None else None
    except: char_len = None
    try: octet_len = int(octet_len) if octet_len is not None else None
    except: octet_len = None
  ...
  for table, idxs in existing.items():
    for idxName, info in idxs.items():
      ...
      for (col, sub) in info["cols"]:
        if sub is None:
          new_cols.append((col, None)); continue
        meta = colMeta.get((table, col), {})
        char_len = meta.get("char_len"); dtype = meta.get("data_type", "")
...
def _fetchExistingOurPrefix(cur):
  ...
  for r in rows:
    if isinstance(r, dict):
      ...
      table, idx, nonu, seq, col, sub = r; nonu = int(nonu); seq = int(seq)
    if sub is not None:
      try: sub = int(sub)
      except: sub = None
...
def _cmpCols(a, b):
  ...
  for (c1, s1), (c2, s2) in zip(a, b):
    if c1 != c2 or s1 != s2: return False
  return True

def _findFunctionalDuplicate(allIdxMap, table, desiredCols, desiredNonUnique, skipName):
  for name, info in allIdxMap.get(table, {}).items():
    if name == skipName: continue
    if int(info.get("non_unique", 1)) != int(desiredNonUnique): continue
    if _cmpCols(info.get("cols", []), desiredCols): return name
  return None
...
def initializeIndexes(db):
  ...
  # Build desired map and flatten for reporting
  desired = _desiredMap()
  total_desired = sum(len(v) for v in desired.values())

  with db.cursor() as cur:
    colMeta = _getColMeta(cur)
    before_map = _fetchExistingOurPrefix(cur); _normalizeExisting(before_map, colMeta)
    before_pairs = {(t, n): info for t, m in before_map.items() for n, info in m.items()}

  # Run the sync operation (creates/drops/updates as needed)
  syncIndexes(db)

  with db.cursor() as cur:
    colMeta = _getColMeta(cur)
    after_map = _fetchExistingOurPrefix(cur); _normalizeExisting(after_map, colMeta)
    after_pairs = {(t, n): info for t, m in after_map.items() for n, info in m.items()}
    all_after = _fetchAllIndexes(cur); _normalizeExisting(all_after, colMeta)

  # Compute deltas
  before_keys = set(before_pairs.keys())
  after_keys  = set(after_pairs.keys())

  created_new = len(after_keys - before_keys)
  dropped = len(before_keys - after_keys)

  # Modified (same name existed before and after but definition changed)
  modified = 0
  unchanged = 0
  for key in (before_keys & after_keys):
    if not _cmpCols(before_pairs[key]["cols"], after_pairs[key]["cols"]) or \
        int(before_pairs[key]["non_unique"]) != int(after_pairs[key]["non_unique"]):
      modified += 1
    else:
      unchanged += 1

  # Skipped due to functional duplicates (desired exists under different name)
  skipped_dupes = 0
  for table, wantByName in desired.items():
    for idxName, want in wantByName.items():
      if (table, idxName) in after_keys:
        continue
      # If an equivalent index exists with a different name, count as duplicate skipped
      dupe = _findFunctionalDuplicate(all_after, table, want["cols"], want["non_unique"], skipName=idxName)
      if dupe:
        skipped_dupes += 1

  logger.info(
    "Indexes sync complete: desired=%s, created=%s, modified=%s, dropped=%s, unchanged=%s, skipped_duplicates=%s.",
    total_desired, created_new, modified, dropped, unchanged, skipped_dupes
  )
